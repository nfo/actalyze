...
TBRUM.q.push = function() {
	...
	if ("conv" == cmd) {
		...
		if (TBRUM.bDone) TBRUM.beaconWrapper();
	} else if ("tags" === cmd) {
		if (TBRUM.tags) TBRUM.tags += ',';
		else TBRUM.tags = '';
		TBRUM.tags += arguments[1];
	}
}
...
TBRUM.done = function(callback) {
	...
	if (TBRUM.autorun && !TBRUM.bDone) {
		TBRUM.beaconWrapper();
		if (TBRUM.measures['onload_t'] && TBRUM.measures['onload_t'] > 0) onload_time = TBRUM.measures['onload_t'];
		else if (TBRUM.measures['onload'] && TBRUM.measures['onload'] > 0) onload_time = TBRUM.measures['onload'];
...
TBRUM.beaconWrapper = function() {
	TBRUM.sendBeacon(false, {
		src: TBRUM.src,
		tbtim: TBRUM.getCookie('TBTIM'),
		conversion: TBRUM.conversions || '',
		tags: TBRUM.tags || ''
	});
}
...
TBRUM.findStartWebTiming = function() {
	...
	if (navigator.userAgent.match(/Firefox/i)) return false;
	...
	for (var i = 0; i < aCookies.length; i++) {
		if (0 === aCookies[i].indexOf("TBRUM=")) {
			...
			if (bReferrerMatch && startTime) {
				TBRUM.dprint("TBRUM.findStartCookie: startTime = " + startTime);
				TBRUM.src = 'cook';
				return startTime;
			}
		}
